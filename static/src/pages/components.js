...
const Header = {
  ...
  template: `
    <div class="header">
      <img src="选手选填部分" />
      <h3>选手选填部分</h3>
      <span v-if="login" class="user-name">选手选填部分</span>
    </div>
    `
  }
...
const mixin = {
  ...
  methods: {
    ...
    handlePopover(data) {
      ...
      axios({
        method: 'get',
        url: `http://localhost:8080/food?traceNumber=${data.traceNumber}`
      })
      .then(ret => {
    ...
    addVegetable() {
      ...
      axios({
        method: 'get',
        url: `http://localhost:8080/trace?traceNumber=${this.traceNumber}`
      })
      .then(ret => {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          console.log(valid);
          this.$emit('confirmPopup', formName)
        } else {
          return false;
        }
      });
